...
def load_dem_from_bytes(data: bytes) -> Tuple[np.ndarray, float, float, Optional[float]]:
    """Load DEM from bytes using MemoryFile for in-memory processing.
    
    Args:
        data: Raw GeoTIFF bytes
        
    Returns:
        Tuple of (elevation_array, resx, resy, nodata)
        
    Raises:
        rasterio.errors.RasterioIOError: If data is not a valid GeoTIFF
    """
    with MemoryFile(data) as memfile:
        with memfile.open() as src:
            ...
            # Get pixel spacing (assuming square pixels for simplicity)
            transform = src.transform
            resx = abs(transform.a)  # Pixel width
            resy = abs(transform.e)  # Pixel height
            
            # Get nodata value
            nodata = src.nodata
            
    return elevation, resx, resy, nodata
