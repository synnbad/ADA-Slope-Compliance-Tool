...
def check_crs_consistency(raster_fp, points_fp, elevation_points_fp):
    ...
    try:
        with rasterio.open(raster_fp) as raster:
            ...
            logger.info("Raster CRS: %s", raster_crs.to_string() if raster_crs else "None")
    ...
    try:
        ...
        logger.info("Original Points CRS: %s", gdf_points.crs.to_string() if gdf_points.crs else "None")
    except Exception as e:
    ...
    try:
        ...
        logger.info("Elevation-Sampled Points CRS: %s", gdf_elevated.crs.to_string() if gdf_elevated.crs else "None")
    except Exception as e:
